...
const schemas = {
  list: Schema.object({}),
  'add-skill': Schema.object({ directory: text, group: Schema.string().required(), revision: text }),
  ...
  'set-skill-group': Schema.object({ id: text, group: Schema.string().required(), revision: text }),
...
export function createWebHandler(store, ctx) {
  return async (endpoint, payload, signal) => {
    try {
      ...
      if (endpoint === 'add-skill') await store.addSkillPath(args.directory, args.group, signal, args.revision)
      if (endpoint === 'add-mcp') {
      ...
      if (endpoint === 'set-skill-group') await store.setSkillGroup(args.id, args.group, signal, args.revision)
