...
import { createSkill, inspectSkills, setSkillGroup, setSkillInvocation } from './skills.js'
...
export function createWebHandler(store, ctx) {
  return async (endpoint, payload, signal) => {
    try {
      ...
      if (endpoint === 'create-skill' || endpoint === 'set-skill-invocation' || endpoint === 'set-skill-group') {
        const rows = await store.read()
        ...
        else await setSkillGroup(rows, args)
      }
